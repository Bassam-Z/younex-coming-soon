...
function enhanceDocument(html) {
  ...
  if (!output.includes('/locales/ar.json')) {
    output = output.replace('</head>', '  <link rel="preload" href="/locales/ar.json?v=33" as="fetch" crossorigin>\n</head>');
  }
  if (!output.includes('/locales/en.json')) output = output.replace('</head>', '  <link rel="preload" href="/locales/en.json?v=33" as="fetch" crossorigin>\n</head>');
  if (!output.includes('products-data.js')) output = output.replace('</head>', '  <script src="/products-data.js?v=5" defer></script>\n</head>');
  if (!output.includes('/i18n.js')) output = output.replace('</head>', '  <script src="/i18n.js?v=6" defer></script>\n</head>');
  if (!output.includes('/inquiry-cart.js')) output = output.replace('</head>', '  <script src="/inquiry-cart.js?v=2" defer></script>\n</head>');
  output = output.replace(/\/locales\/(ar|en)\.json\?v=\d+/g, '/locales/$1.json?v=33');
